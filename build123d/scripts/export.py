...
presets and writes exactly two artifacts per preset into TARGET_DIR:

    TARGET_DIR/<model-slug>/<preset-id>.stl
    TARGET_DIR/<model-slug>/<preset-id>.glb
...
Contract (validated by tests/test_presets_bake.py):
  ...
  - no PNGs in this mode (bake inputs only; PNGs are review artifacts).
"""
import argparse
import sys
...
def export_presets_only(target: Path) -> int:
    """Bake every preset of every app-listed model into target/<slug>/<preset-id>.{stl,glb}."""
    ...
    for spec in specs:
        ...
        for preset in spec.presets:
            ...
            export_gltf(part, str(glb), binary=True)
            baked += 1
            print(f"{spec.name}/{preset.id}: vol={part.volume:.0f}mm3 -> {stl}, {glb}")
