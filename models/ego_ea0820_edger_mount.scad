...
// (multiconnect variant: Z = backer_thickness (6.5) + mesh_h (152.4) = 158.9)
...
// LOW y is usage-UP for this model (the -Y-up axis — opposite
// ego_lb6500's +Y-up), so the closed retention domes take the cantilever
// load exactly like the directional snaps' strong nub does. The slab is
// thickened by mc_weld so its top (z=bt) sinks mc_weld into the plate's
// flat build face (at z=body_lift=backer_thickness) — a real weld, not a
// face-kiss.
module backer_panel() {
