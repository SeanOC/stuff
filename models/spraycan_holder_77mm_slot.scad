// Multiboard-mounted holder for 77mm spray can — Multiconnect slot variant.
// Larger-diameter sibling of spraycan_holder_70mm_slot (st-rw2). Copied
// rather than parameterized so the 70mm STL output stays byte-identical
// until both variants have shipped successfully.
...
can_diameter       = 77;     // @param number min=30 max=200 step=0.5 label="Can diameter (mm)"
...
// gusset_back_w widened from 28 → 32 to brace the larger ring: ring_od
// grew from 77.5 → 84.5 (~9% more diameter, ~18% more mass) and its
// center shifted ~3.5mm further from the backer, increasing the
// cantilever moment on the backer-to-cradle joint.
outer_chamfer      = 1.0;    // outer/touchable edges (top of ring, bottom of cup)
inner_chamfer      = 0.5;    // can-entry lead-in at top of bore
gusset_back_w      = 32;     // trapezoid width where gusset meets backer (mm)
gusset_front_w     = 10;     // trapezoid width where gusset meets ring wall (mm)
gusset_depth       = 6;      // gusset reach from backer into +Y (mm)

// === Derived ===
ring_id       = can_diameter + 2 * clearance;               // 78.5
ring_od       = ring_id + 2 * wall;                         // 84.5
...
// X: max(backer_w=50, ring_od=84.5) = 84.5
// Y: backer_thickness(6.5) + ring_od(84.5) = 91
// Z: backer_height(80) > cup_depth+ring_height(40)
PRINT_ANCHOR_BBOX = [84.5, 91, 80];
